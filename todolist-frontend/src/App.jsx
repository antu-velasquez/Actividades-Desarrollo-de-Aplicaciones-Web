...
import { fetchGoals, addGoalThunk, removeGoalThunk } from './store';
...
function App() {
  ...
  const metas = useSelector((state) => state.goals?.items || []);
  ...
  useEffect(() => {
    ...
    if (vista === 'tareas') {
      ...
      dispatch(fetchGoals())
        .unwrap()
        .catch((err) => setError(err.message || "Error al conectar con el servidor de metas"));
    }
  }, [vista, dispatch]);
  ...
  const obtenerTareas = async () => {
    try {
      ...
      setError(err.message || "No se pudo conectar al backend local");
    }
  };
    ...
    if (vista === 'tareas') {
      try {
        const response = await fetch(`${API_URL}/addTask`, {
          ...
          body: JSON.stringify({ task: inputData.nombre, description: inputData.descripcion, dueDate: inputData.fecha })
        });
        ...
        setSuccess(`Status 200: Tarea guardada en base de datos`);
        setTimeout(() => {
          obtenerTareas();
      ...
      dispatch(addGoalThunk({ goal: inputData.nombre, description: inputData.descripcion, dueDate: inputData.fecha }))
        .unwrap()
        .then(() => {
          setSuccess(`Status 200: Meta guardada en base de datos`);
          setTimeout(() => {
            setInputData({ nombre: '', descripcion: '', fecha: '' });
            handleClose();
          }, 1500);
  ...
  const handleEliminar = async (id) => {
    if (vista === 'tareas') {
      try {
        const response = await fetch(`${API_URL}/removeTask`, {
          method: 'DELETE',
          headers: { 'Content-Type': 'application/json', 'Authorization': API_KEY },
        ...
        setSuccess(`Status 200: Tarea eliminada de la base de datos`);
        obtenerTareas();
      ...
      dispatch(removeGoalThunk(id))
        .unwrap()
        .then(() => {
          setSuccess(`Status 200: Meta eliminada de la base de datos`);
          setTimeout(() => setSuccess(null), 3000);
        })
        .catch((err) => setError(err.message));
    }
          ...
          <Col xs={12} lg={8}>
            ...
            {listaActual.length === 0 ? (
              ...
              listaActual.map((item) => (
                <Card key={item._id} className="tarea-item shadow-sm mb-3">
                  <Card.Body className="d-flex justify-content-between align-items-center">
                    <div>
                      <h5 className="mb-1 fw-bold">{item.title}</h5>
                      {item.description && <p className="mb-1 text-secondary small">{item.description}</p>}
                      {item.dueDate && <p className="mb-1 text-danger small">Límite: {item.dueDate}</p>}
                      <p className="text-muted extra-small mb-0">DB ID: {item._id}</p>
                    </div>
                    <Button variant="outline-danger" className="rounded-pill" onClick={() => handleEliminar(item._id)}>Eliminar</Button>
                  </Card.Body>
                </Card>
              ))
            )}
